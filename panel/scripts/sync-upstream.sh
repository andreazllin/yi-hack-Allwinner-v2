#!/bin/sh
# Rebase the panel branch onto upstream, refusing to proceed silently if the
# CGI backend changed.
#
#   sh panel/scripts/sync-upstream.sh
#
...
# note: a one-line edit to a CGI script can silently break the panel, and
...
#     (keep the panel's two-call save anyway: harmless against fixed
...
if git diff --quiet "$OLD..upstream/master" -- "$CGI_PATH"; then
    echo "none - the API the panel talks to is unchanged."
else
    git diff "$OLD..upstream/master" -- "$CGI_PATH"
    echo
    echo "REFUSING TO REBASE: the CGI backend changed."
    echo "Review the diff above against panel/yi-hack-openapi.yaml, update the"
    echo "spec and run 'pnpm generate:sdk', then re-run this script."
    echo "Override deliberately with: SKIP_CGI_CHECK=1 sh panel/scripts/sync-upstream.sh"
...
echo
echo "Done. Remember the firmware flashed on the camera is what the panel"
