// Mock yi-hack-Allwinner-v2 camera, as a transport-free request handler.
//
// This file holds the whole emulation and knows nothing about how the request
// reached it. Two adapters call it:
//   - mock/camera.mjs      a node:http server, for `pnpm dev` against a proxy
//   - src/lib/mock/start.ts  a Service Worker (msw), for the Pages demo build
// Nothing here may use node: builtins, or the demo build breaks.
//
// Emulates the BusyBox httpd CGI shell scripts documented in
// frontend/yi-hack-openapi.yaml, INCLUDING their hostile behaviors:
...
//   - reboot.sh replies, then refuses every request for ~10 seconds
//
// See mock/camera.mjs for how to run it standalone.
...
// Uint8Array, not Buffer: this file also runs in the browser.
const bytes = (...parts) => {
	const flat = parts.flatMap((part) =>
		typeof part === "number"
			? Array(part).fill(0) // n zero bytes
			: [...(part.match(/../g) ?? [])].map((b) => Number.parseInt(b, 16)),
	);
	return Uint8Array.from(flat);
};
const JPEG = Uint8Array.from(atob(JPEG_B64), (c) => c.charCodeAt(0));
// Minimal ftyp+mdat stub; valid box structure, not playable.
const MP4 = bytes(
	"000000146674797069736f6d0000020069736f6d", // ftyp
	"000000106d646174",
	8, // mdat
);
const SEVENZ = bytes("377abcaf271c", 26);
...
const SEED = {
	camera:
		conf(`SWITCH_ON=yes;SAVE_VIDEO_ON_MOTION=yes;MOTION_DETECTION=no;SENSITIVITY=low
	...
	system:
		conf(`HTTPD=yes;TELNETD=yes;SSHD=yes;FTPD=yes;BUSYBOX_FTPD=no;MDNSD=yes;DISABLE_CLOUD=no
	...
	mqtt_advertise:
		conf(`MQTT_ADV_LINK_ENABLE=no;MQTT_ADV_LINK_BOOT=no;MQTT_ADV_LINK_CRON=no
	...
	timelapse = [
		{
			...
			time: "Time: 12:00:00",
			filename: "2026-08-22_12-00-00.avi",
		},
		{
			date: "Date: 2026-08-23",
			time: "Time: 06:30:00",
			filename: "2026-08-23_06-30-00.avi",
		},
	];
}
resetState();
...
const rawQuery = (ctx) => ctx.url.split("?")[1] ?? "";
...
// Records the reply on ctx instead of writing it: the adapter decides how a
// status, a header map and a string-or-bytes body reach the client. A handler
// that calls send() twice keeps the first reply, like a script that has
// already flushed its headers.
...
async function cameraSettings(ctx) {
	const params = parseParams(rawQuery(ctx), 15);
	const binary = {
		switch_on: "SWITCH_ON",
		save_video_on_motion: "SAVE_VIDEO_ON_MOTION",
		ai_human_detection: "AI_HUMAN_DETECTION",
		ai_vehicle_detection: "AI_VEHICLE_DETECTION",
		ai_animal_detection: "AI_ANIMAL_DETECTION",
...
function getConfigs(ctx) {
	...
	if (conf === "camera")
		return send(
			ctx,
			confJson(persisted.camera, { HOMEVER: "7.1.00.26A_202101191703" }),
		);
	if (conf === "system")
...
async function setConfigs(ctx) {
	...
	for (const [key, value] of Object.entries(obj)) {
		...
		if (key === "HOSTNAME") {
			hostname = String(value) || "yi-hack";
			continue;
		}
		if (key === "PROXYCHAINS_SERVERS") {
			persisted.proxychains[key] = String(value);
			continue;
		}
		if (
			key === "MOTION_IMAGE_DELAY" &&
			!(NUM_OK.test(String(value)) && Number(value) <= 5.0)
		)
			continue;
		if (key === "TIMELAPSE_DT") {
			const ok =
				...
				(/^1440\+\d+$/.test(String(value)) &&
					Number(String(value).slice(5)) <= 1440);
...
function statusJson(ctx) {
	...
	const body =
		...
		[
			...
			["hostname", hostname],
			["fw_version", "0.5.4"],
			["home_version", "7.1.00.26A_202101191703"],
			["model_suffix", "h51ga"],
			["ptz", "yes"],
			["go2rtc", "no"],
			["serial_number", "MOCK0123456789ABCDEF"],
			["local_time", new Date().toString()],
			["uptime", `${Math.floor(up / 86400)} days, ${hh}:${mm}`],
			["load_avg", "0.42, 0.35, 0.30"],
			["total_memory", "73048 kB"],
			["free_memory", "18204 kB"],
			["free_sd", "42%"],
			["local_ip", LOCAL_IP],
			["netmask", "255.255.255.0"],
			["gateway", "192.168.10.1"],
			["mac_addr", "0C:8C:24:A1:B2:C3"],
			["wlan_essid", essid],
			["wlan_strength", "55"],
		]
			.map(([k, v]) => `"${k}":"${v}"`)
...
function eventsDir(ctx) {
	const records = events
		.map(
			(d) =>
				`{"datetime":"Date: ${d.date} Time: ${d.hour}:00","dirname":"${d.dirname}"}`,
		)
		.join(",");
...
function eventsFile(ctx) {
	const raw = rawQuery(ctx);
	if (QS_BLOCK.test(raw) || raw.split("=")[0] !== "dirname")
		return send(ctx, ERR_TRUE);
...
function eventsDirDel(ctx) {
	...
	if (key === "dir")
		events = value === "all" ? [] : events.filter((d) => d.dirname !== value);
...
function ptz(ctx) {
	...
	if (!["step", "abs", "rel", "cont", undefined].includes(p.action))
		return send(ctx, ERR_TRUE);
...
function preset(ctx) {
	const raw = rawQuery(ctx);
	if (QS_BLOCK.test(raw))
		return send(ctx, '{"error":"true","message":"Invalid query"}');
...
async function wifi(ctx) {
	...
	if (action === "save") {
		try {
			...
			await sleep(1000);
			ctx.slept += 1000; // configure_wifi.sh + sleep 1
...
async function speak(ctx) {
	...
	if (p.voldb !== undefined && !NUM_OK.test(p.voldb))
		return send(
			ctx,
			'{"error":"true","description":"Invalid volume (dB)"}',
			null,
		);
	await sleep(1000);
	ctx.slept += 1000;
...
async function speakerFile(ctx) {
	const file = ctx.body.split("\n")[0];
	if (!file)
		// faithful malformed body: missing comma, like the real script
...
function timelapseRoute(ctx) {
	...
	if (p.action === "list") {
		const records = timelapse
			.map(
				(r) =>
					`{"date":"${r.date}","time":"${r.time}","filename":"${r.filename}"}`,
			)
			.join(",");
...
async function fwUpgrade(ctx) {
	...
	if (mode === "info")
		// local_fw is the API's ONE bare JSON boolean
		return send(
			ctx,
	...
	if (mode === "upgrade") {
		...
		// microseconds hides every in-flight state the frontend has to get right.
		await sleep(4000);
...
function reboot(ctx) {
	send(ctx, ERR_FALSE);
	// Reply first, then "reboot". The window is enforced in handle(), which
	// refuses everything until it passes; the node adapter turns that refusal
	// back into a dropped socket, the way the real camera behaves.
	setTimeout(() => {
...
function links(ctx) {
	const port =
		persisted.system.HTTPD_PORT === "80"
			? ""
			: `:${persisted.system.HTTPD_PORT}`;
	send(
		ctx,
		JSON.stringify({
...
const routes = {
	...
	"/cgi-bin/speaker.sh": (ctx) =>
		send(ctx, '{"error":"false","description":""}'),
	...
	"/cgi-bin/reset.sh": (ctx) => {
		resetState();
		send(ctx, ERR_FALSE);
	},
	"/cgi-bin/save.sh": (ctx) =>
		send(ctx, SEVENZ, "application/octet-stream", {
	...
	"/cgi-bin/proxy.sh": (ctx) => {
		...
		send(
			ctx,
			'{"error":"false","result":{"ip":"203.0.113.7","city":"Milan","region":"Lombardy","country":"IT","org":"AS64496 MockISP"}}',
		);
	},
	"/cgi-bin/hostname.js": (ctx) =>
		send(ctx, `hostname="${hostname}";`, "text/javascript"),
	"/cgi-bin/links.sh": links,
};

// ------------------------------------------------------------------- handle

// Serve one request. `url` is the path with its query string, exactly as the
// CGI scripts see it ("/cgi-bin/x.sh?a=b"); `body` is the already-read request
// body, "" when there is none.
//
// Returns { status, headers, body, slept } — or { networkError: true } while
// the camera is "rebooting", which an adapter must surface as a transport
// failure and never as an HTTP response.
export async function handle({ url, method = "GET", body = "" }) {
	if (Date.now() < rebootUntil) return { networkError: true };

	const ctx = { url, method, body, slept: 0 };
	const pathname = url.split("?")[0];
	try {
		// Recordings and thumbnails are static files, not CGI.
		if (pathname.startsWith("/record/") && pathname.endsWith(".jpg")) {
			send(ctx, JPEG, "image/jpeg");
		} else if (pathname.startsWith("/record/") && pathname.endsWith(".mp4")) {
			send(ctx, MP4, "video/mp4; charset=utf-8");
		} else if (routes[pathname]) {
			await routes[pathname](ctx);
		}
	} catch (e) {
	...
	return { ...ctx.response, slept: ctx.slept };
}

// One line per request, in the format the node server has always logged.
export function describeRequest(method, url, response) {
	if (response.networkError) return `${method} ${url} -> connection dropped`;
	const { body, headers, slept } = response;
	const preview =
		body instanceof Uint8Array
			? `<${body.length} bytes>`
			: String(body ?? "").slice(0, 80) || "<empty>";
	const type = headers["Content-Type"] ?? "no content-type";
	return `${method} ${url} -> ${type} ${preview}${slept ? ` [slept ${slept}ms]` : ""}`;
}
