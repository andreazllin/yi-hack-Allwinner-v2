#!/usr/bin/env node
// Mock yi-hack-Allwinner-v2 camera server for frontend frontend development.
//
...
const MP4 = Buffer.concat([
	...
	Buffer.from("000000106d646174", "hex"),
	Buffer.alloc(8), // mdat
]);
...
function send(ctx, body, type = "application/json", headers = {}) {
	...
	console.log(
		`${ctx.req.method} ${ctx.req.url} -> ${type || "no content-type"} ${preview}${delay}`,
	);
}
...
function reboot(ctx) {
	send(ctx, ERR_FALSE);
	setTimeout(() => {
		// reply first, then "reboot": ~10s of dropped connections
	...
	try {
		...
		if (pathname.startsWith("/record/")) {
			...
			if (pathname.endsWith(".mp4"))
				return send(ctx, MP4, "video/mp4; charset=utf-8");
		}
