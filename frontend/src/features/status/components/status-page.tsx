...
export const StatusPage: FunctionComponent = () => {
	const { data, isPending, isError, error, refetch } = useStatus();

	// This page polls, so a failure after a good read still has the last good
	// values in `data`. Only take over the page when there is nothing to show;
	// otherwise keep the readings up and put the failure above them, so one
	// dropped poll does not blank a working dashboard.
	if (isError && !data) {
		return (
			<Stack gap="md">
